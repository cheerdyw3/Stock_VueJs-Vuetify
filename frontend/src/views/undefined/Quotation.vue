...
<section>  
  <div v-if="FormSelect">
  <v-card elevation="0" >  <h1>เปิดใบเสนอราคา</h1>
  </v-card>
  <v-card  elevation="0">
    <v-list>
  ...
  <v-card  elevation="0" 
    ...
    max-height="700"
  >  
       <v-card  outlined tile  elevation="2"  v-for="(item,i) in cates" :key="i" class="mb-5" >
    ...
    <v-col  
        ...
        cols="12" absolute
      >
        <v-btn v-if="Carts.length" small color="primary" @click="Create()">พิมพ์ใบเสนอราคา</v-btn> 
      </v-col>  
      
     </div>
     <v-dialog v-model="dialog" scrollable max-width="400px"> 
      <v-card>
        <v-card-title>เปิดใบเสนอราคา</v-card-title>
        <v-divider></v-divider>
        <v-progress-circular indeterminate color="primary" class="mx-auto my-5" v-if="load"></v-progress-circular>
         <v-alert class="mx-5" dense type="error" v-if="skuError">
     ...
     <v-dialog v-model="dialogData" fullscreen hide-overlay transition="dialog-bottom-transition">
      ...
      <v-card>
        <v-toolbar dark dense color="primary">
          ...
          <v-card outlined > 
               <QuotationC :cws="cws" :agentType="agentType" :dataDetail="membersID" :Carts="Carts" @back="back($event)"></QuotationC>
          </v-card> 
      </v-card>
    </v-dialog> 
</section>
...
import QuotationC from '@/components/QuotationC'  
    ...
    methods: { 
       Create(){ 
        ...
        async loadMemberID(sku="") {     
            this.load = true
            try { 
                const response = await fetch(this.API+'/api/v1/agent/membersku/'+sku)
                const data = await response.json()  
                if(data.Error=="error"){
                  this.skuError = true
                  this.load = false
