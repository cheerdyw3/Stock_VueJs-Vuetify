<template> 
    <v-card elevation="0">
      <v-card class="px-5 elevation-3">
        ...
        <v-card   class="mt-5 mx-auto px-5 elevation-3">
            <v-container class="fill-height" fluid>
                <v-row no-gutters>
        ...
        <v-card   class="mt-5 mx-auto px-5 elevation-3">
            <v-container class="fill-height" fluid>
                ...
                <formMemberD :dataFormA="data.dataA" :dataFormC="data.dataC" @dataFormD="dataFormD($event)"
                    v-if="form.FormC"></formMemberD>
            </v-container>
        </v-card>
        <v-card   class="mt-5 mx-auto px-5 elevation-3">
            <v-container class="fill-height" fluid>
    ...
    methods: {
        ...
        async sendData() {  
            ...
            this.data.dataBranch = Branch.all();
             this.data.dataM = Admin.all();
            try {
              // Create request to api service
             const req = await fetch(this.API+"/api/v1/agent/newmember", {
                  method: 'POST',  
