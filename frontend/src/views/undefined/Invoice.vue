...
<section> 
  <div v-if="FormSelect">
  <v-card elevation="0" ><h1>ใบส่งสินค้า</h1>
  </v-card>
  <v-card  elevation="0">
    <v-list>
  ...
  >  
    ...
    <v-col  
        ...
        cols="12" absolute
      >
        <v-btn v-if="Carts.length" small color="primary" @click="Create()">พิมพ์ใบส่งสินค้า</v-btn> 
      </v-col>  
      
     </div>
     <v-dialog v-model="dialog" scrollable max-width="400px"> 
      <v-card>
        <v-card-title>เปิดใบส่งสินค้า</v-card-title>
        <v-divider></v-divider>
     ...
     <v-dialog v-model="dialogData" fullscreen hide-overlay transition="dialog-bottom-transition">
      ...
      <v-card>
        <v-toolbar dark dense color="primary">
          ...
          <v-card> 
               <InvoiceC :cws="cws" :agentType="agentType" :dataDetail="membersID" :Carts="Carts" @back="back($event)"></InvoiceC>
          </v-card>
      </v-card>
    </v-dialog>
...
import InvoiceC from '@/components/InvoiceC'  
